...
''' <summary>
''' 特権ユーザーログイン（ひびきユーザー登録）画面Sqlクラス
''' </summary>
''' <remarks>特権ユーザーログイン（ひびきユーザー登録）画面のSQLの作成・設定を行う
''' <para>作成情報：2012/08/30 k.imayama
''' <p>改訂情報:</p>
''' </para></remarks>
Public Class SqlHBKX0101

    '所属マスター取得SQL
    Private strSelectSzkMtb As String = "SELECT" & _
                                        " sm.HBKUsrID," & _
                                        " sm.GroupCD" & _
                                        " FROM szk_mtb AS sm" & _
                                        " WHERE sm.JtiFlg <> '1'" & _
                                        " AND sm.HBKUsrID = :HBKUsrID" & _
                                        " AND sm.GroupCD = :GroupCD" & _
                                        " AND sm.UsrGroupFlg = '1'"
    ...
    ''' <param name="dataHBKX0101">[IN]特権ユーザーログイン（ひびきユーザー登録）画面データクラス</param>
    ''' <returns>boolean エラーコード  true 正常終了  false	異常終了 </returns>
    ''' <remarks>所属マスターから該当データの取得用SQLをアダプタに設定する
    ''' <para>作成情報：2012/10/15 s.yamaguchi
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Public Function SetSelectSzkMtbSql(ByRef Adapter As NpgsqlDataAdapter, ByVal Cn As NpgsqlConnection, ByVal dataHBKX0101 As DataHBKX0101) As Boolean
        ...
        Try
            ...
            Adapter.SelectCommand.Parameters("HBKUsrID").Value = PropUserId
            Adapter.SelectCommand.Parameters("GroupCD").Value = PropWorkGroupCD
    ...
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Public Function SetSelectSuperUserSql(ByRef Adapter As NpgsqlDataAdapter, ByVal Cn As NpgsqlConnection, ByVal dataHBKX0101 As DataHBKX0101) As Boolean
        ...
        Try
            ...
            Adapter.SelectCommand.Parameters("SuperUsrID").Value = dataHBKX0101.PropTxtUserId.Text
            Adapter.SelectCommand.Parameters("Password").Value = dataHBKX0101.PropTxtPassword.Text
    ...
    ''' <para>作成情報：2012/08/30 k.imayama
    ''' </para></remarks>
    Public Function SetInsertSuperLoginLogSql(ByRef Cmd As NpgsqlCommand, ByVal Cn As NpgsqlConnection, ByVal dataHBKX0101 As DataHBKX0101) As Boolean
        ...
        Try
            ...
            'バインド変数に値をセット
            With Cmd
                .Parameters("SuperUsrID").Value = dataHBKX0101.PropTxtUserId.Text
                .Parameters("HBKUsrID").Value = CommonHBK.CommonDeclareHBK.PropUserId
                .Parameters("LogInOutKbn").Value = dataHBKX0101.PropStrLogInOutKbn
                .Parameters("ClientHostNM").Value = Dns.GetHostName()
                .Parameters("ProgramID").Value = dataHBKX0101.PropStrProgramID
            End With
