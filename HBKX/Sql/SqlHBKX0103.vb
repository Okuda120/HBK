...
''' <summary>
''' 特権ユーザーログイン（エンドユーザ取込）画面Sqlクラス
''' </summary>
''' <remarks>特権ユーザーログイン（エンドユーザ取込）画面のSQLの作成・設定を行う
''' <para>作成情報：2012/08/30 k.imayama
''' <p>改訂情報:</p>
''' </para></remarks>
Public Class SqlHBKX0103

    '特権ユーザーマスター（エンドユーザ取込）取得SQL
    Private strSelectSuperUser As String = "SELECT" & _
                                            " sm.SuperUsrID " & _
                                            ",sm.Password " & _
                                            "FROM superusr_mtb sm " & _
                                            "WHERE sm.JtiFlg <> '1' " & _
                                            " AND sm.EndUsrImpFlg = '1' " & _
    ...
    ''' <summary>
    ''' 特権ユーザーマスター（エンドユーザ取込）取得SQL作成
    ''' </summary>
    ''' <param name="Adapter">[IN/OUT]NpgSqlDataAdapterクラス</param>
    ''' <param name="Cn">[IN]NpgSqlConnectionクラス</param>
    ''' <param name="dataHBKX0103">[IN]特権ユーザーログイン（エンドユーザ取込）画面データクラス</param>
    ''' <returns>boolean エラーコード  true 正常終了  false	異常終了 </returns>
    ''' <remarks>特権ユーザーマスターから該当IDの取得用SQLをアダプタに設定する
    ''' <para>作成情報：2012/08/30 k.imayama
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Public Function SetSelectSuperUserSql(ByRef Adapter As NpgsqlDataAdapter, ByVal Cn As NpgsqlConnection, ByVal dataHBKX0103 As DataHBKX0103) As Boolean
        ...
        Try
            ...
            Adapter.SelectCommand.Parameters("SuperUsrID").Value = dataHBKX0103.PropTxtUserId.Text
            Adapter.SelectCommand.Parameters("Password").Value = dataHBKX0103.PropTxtPassword.Text
    ...
    ''' </para></remarks>
    Public Function SetInsertSuperLoginLogSql(ByRef Cmd As NpgsqlCommand, ByVal Cn As NpgsqlConnection, ByVal dataHBKX0103 As DataHBKX0103) As Boolean
        ...
        Try
            ...
            'バインド変数に値をセット
            With Cmd
                .Parameters("SuperUsrID").Value = dataHBKX0103.PropTxtUserId.Text
                .Parameters("HBKUsrID").Value = CommonHBK.CommonDeclareHBK.PropUserId
                .Parameters("LogInOutKbn").Value = dataHBKX0103.PropStrLogInOutKbn
                .Parameters("ClientHostNM").Value = Dns.GetHostName()
                .Parameters("ProgramID").Value = dataHBKX0103.PropStrProgramID
            End With
