...
''' <summary>
''' インシデント登録（貸出誓約書出力）Sqlクラス
''' </summary>
''' <remarks>インシデント登録（貸出誓約書出力）のSQLの作成・設定を行う
''' <para>作成情報：2012/07/23 s.tsuruta
''' <p>改訂情報:</p>
''' </para></remarks>
Public Class SqlHBKC0202
    ...
    ''' <param name="dataHBKC0202">[IN]インシデント登録（貸出誓約書）データクラス</param>
    ''' <returns>boolean エラーコード  true 正常終了  false	異常終了 </returns>
    ''' <remarks>CIサポセン機器履歴 取得用のSQLを作成し、アダプタにセットする
    ''' <para>作成情報：2012/07/23 s.tsuruta
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Public Function SetSelectCISupportSql(ByRef Adapter As NpgsqlDataAdapter, _
                                          ByVal Cn As NpgsqlConnection, _
                                          ByVal dataHBKC0202 As DataHBKC0202) As Boolean
        ...
        Try
            ...
            Adapter.SelectCommand.Parameters("CINmb").Value = dataHBKC0202.PropIntCINmb                                 'CI番号
            Adapter.SelectCommand.Parameters("RirekiNo").Value = dataHBKC0202.PropIntRirekiNo                           '履歴番号
