...
Imports Npgsql
''' <summary>
''' インシデント登録画面(返却確認書出力)SQLクラス
''' </summary>
''' <remarks>インシデント登録画面(返却確認書出力)のSQLを定義したクラス
''' <para>作成情報：2012/07/26k.ueda
''' <p>改定情報：</p>
''' </para></remarks>
Public Class SqlHBKC0205
    ...
    ''' <param name="dataHBKC0205">[IN]インシデント登録（返却確認書出力)データクラス</param>
    ...
    Public Function SetSelectCISupportSql(ByRef Adapter As NpgsqlDataAdapter, _
                                                   ...
                                                   ByVal dataHBKC0205 As DataHBKC0205) As Boolean
        ...
        Try
            ...
            Adapter.SelectCommand.Parameters("CINmb").Value = dataHBKC0205.PropIntCINmb
            '履歴番号
            Adapter.SelectCommand.Parameters.Add(New NpgsqlParameter("RirekiNo", NpgsqlTypes.NpgsqlDbType.Integer))
            Adapter.SelectCommand.Parameters("RirekiNo").Value = dataHBKC0205.PropIntRirekiNo
