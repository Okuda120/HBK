...
''' <summary>
''' 問題検索一覧Excel出力ロジッククラス
''' </summary>
''' <remarks>問題検索一覧Excel出力Logicクラス
''' <para>作成情報：2012/08/13 y.ikushima
''' <p>改定情報：</p>
''' </para></remarks>
Public Class LogicHBKD0102

    'インスタンス作成
    Private sqlHBKD0102 As New SqlHBKD0102
    Private commonLogic As New CommonLogic
    Private commonLogicHBK As New CommonLogicHBK

    ''' <summary>
    ''' 問題検索一覧（EXCEL出力）メイン処理
    ''' </summary>
    ...
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Public Function CreateOutPutFileMain(ByRef dataHBKD0102 As DataHBKD0102) As Boolean

        '開始ログ出力
        commonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)

        '問題データ検索
        If CreateOutFileForSearch(dataHBKD0102) = False Then
            Return False
        End If

        'ファイル作成処理
        If SetOutPutDataForExcelProblem(dataHBKD0102) = False Then
            Return False
        End If
    ...
    ''' <param name="dataHBKD0102">[IN/OUT]問題検索一覧Excel出力Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>問題検索一覧出出力処理を行う
    ''' <para>作成情報：2012/08/13 y.ikushima
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Public Function CreateOutFileForSearch(ByRef dataHBKD0102 As DataHBKD0102) As Boolean
        '開始ログ出力
        commonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)
        ...
        Try
            ...
            '問題検索一覧データ取得SQLの作成・設定
            If sqlHBKD0102.SetSelectProblemInfoSql(Adapter, Cn, dataHBKD0102) = False Then
                Return False
            End If

            'ログ出力
            commonLogic.WriteLog(Common.LogLevel.DEBUG_Lv, "問題検索一覧データ取得", Nothing, Adapter.SelectCommand)

            'データを取得
            Adapter.Fill(dtOutPut)

            'データクラスに保存
            dataHBKD0102.PropDtResult = dtOutPut
    ...
    ''' <summary>
    ''' 問題検索一覧Excel出力データ出力処理
    ''' </summary>
    ''' <param name="dataHBKD0102">[IN/OUT]問題検索一覧Excel出力Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>DBから取得した問題検索一覧Excel出力データをExcelにセットする
    ''' <para>作成情報：2012/08/03 y.ikushima
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Public Function SetOutPutDataForExcelProblem(ByRef dataHBKD0102 As DataHBKD0102) As Boolean
        '開始ログ出力
        commonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)
        ...
        Dim strOutPutFilePath As String = dataHBKD0102.PropStrOutPutFilePath '出力先ファイルパス
        ...
        strFormatFilePath = Path.Combine(strFormatFilePath, FORMAT_PROBLEM_SEARCH)
        ...
        Try
            ...
            'データ行数分ループ
            With dataHBKD0102.PropDtResult
            ...
            strOutPutLogFileName = PropUserId & "_" & _
                                    ...
                                    dataHBKD0102.PropStrOutPutFileName
        ...
        Finally
            ...
            dataHBKD0102.PropDtResult.Dispose()
        End Try
    End Function
