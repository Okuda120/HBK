...
''' <summary>
''' インシデント登録（貸出誓約書出力）ロジッククラス
''' </summary>
''' <remarks>インシデント登録（貸出誓約書出力）のロジッククラス
''' <para>作成情報：2012/07/23 s.tsuruta
''' <p>改定情報：</p>
''' </para></remarks>
Public Class LogicHBKC0202

    'インスタンス生成
    Private sqlHBKC0202 As New SqlHBKC0202
    ...
    ''' 画面初期表示メイン処理
    ''' </summary>
    ''' <param name="dataHBKC0202">[IN/OUT]インシデント登録（貸出誓約書出力）Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>インシデント登録（貸出誓約書出力）に初期データをセットする
    ''' <para>作成情報：2012/07/23 s.tsuruta
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Public Function InitMain(ByRef dataHBKC0202 As DataHBKC0202) As Boolean

        '開始ログ出力
        commonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)

        'データ取得処理
        If GetInitData(dataHBKC0202) = False Then
            Return False
        End If

        'Excelデータ出力処理
        If OutputExcelFile(dataHBKC0202) = False Then
            Return False
        End If
    ...
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Private Function GetInitData(ByRef dataHBKC0202 As DataHBKC0202) As Boolean
        ...
        Try
            ...
            'CIサポセン機器（保存用）取得処理
            If GetCISupport(Adapter, Cn, dataHBKC0202) = False Then
                Return False
            End If

            '複数人利用（保存用）取得処理
            If GetShare(Adapter, Cn, dataHBKC0202) = False Then
                Return False
            End If
    ...
    ''' <param name="dataHBKC0202">[IN/OUT]インシデント登録（貸出誓約書出力）Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>インシデント登録（貸出誓約書出力）に必要なCIサポセン機器履歴テーブルを取得する
    ''' <para>作成情報：2012/07/23 s.tsuruta
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Private Function GetCISupport(ByVal Adapter As NpgsqlDataAdapter, _
                                          ByVal Cn As NpgsqlConnection, _
                                          ByRef dataHBKC0202 As DataHBKC0202) As Boolean
        ...
        Try
            ...
            'SQLの作成・設定
            If sqlHBKC0202.SetSelectCISupportSql(Adapter, Cn, dataHBKC0202) = False Then
                Return False
            End If

            'ログ出力
            commonLogic.WriteLog(Common.LogLevel.DEBUG_Lv, "CIサポセン機器履歴取得", Nothing, Adapter.SelectCommand)

            'データを取得
            Adapter.Fill(dtCISupport)

            '取得データをデータクラスにセット
            dataHBKC0202.PropDtCISupport = dtCISupport
    ...
    ''' <param name="dataHBKC0202">[IN/OUT]インシデント登録（貸出誓約書出力）Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>インシデント登録（貸出誓約書出力）に必要な複数人利用履歴テーブルを取得する
    ...
    Private Function GetShare(ByVal Adapter As NpgsqlDataAdapter, _
        ...
        Try
            ...
            'SQLの作成・設定
            If sqlHBKC0202.SetSelectShareSql(Adapter, Cn, dataHBKC0202) = False Then
                Return False
            End If

            'ログ出力
            commonLogic.WriteLog(Common.LogLevel.DEBUG_Lv, "複数人利用履歴取得", Nothing, Adapter.SelectCommand)

            'データを取得
            Adapter.Fill(dtShare)

            '取得データをデータクラスにセット
            dataHBKC0202.PropDtShare = dtShare
    ...
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Private Function OutputExcelFile(ByRef dataHBKC0202 As DataHBKC0202) As Boolean
        ...
        Try
            ...
            'フォーマットファイルパスの設定
            strFormatFilePath = Path.Combine(Application.StartupPath, FORMAT_FOLDER_PATH, FORMAT_INCIDENT_KASHIDASHI)
            ...
            If OutputAgreeLendPC(xlSheets, xlSheet, dataHBKC0202) = False Then
                Return False
            End If

            'agree_lend_tokenの操作を行う(セルに値をセット)
            If OutputAgreeLendToken(xlSheets, xlSheet, dataHBKC0202) = False Then
                Return False
            End If

            'agree_lend_otherの操作を行う(セルに値をセット)
            If OutputAgreeLendOther(xlSheets, xlSheet, dataHBKC0202) = False Then
                Return False
            End If
    ...
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Private Function OutputAgreeLendPC(ByVal xlSheets As Object, _
        ...
        Try
            ...
            '対象シートをセット
            xlSheet = xlSheets(SHEETNAME_KASHIDASHI_PC)                                                         'シート名（PC）
            ...
            With dataHBKC0202
                ...
                If .PropDtShare.Rows.Count > 0 Then
                    ...
                    'ShareUsrセルの1行下のセルにユーザーID+ユーザー名+部署の出力
                    ...
                    For index = 1 To .PropDtShare.Rows.Count - 1
                        ...
                        xlSheet.Range(CELLNAME_SHARE).Offset(1 + (index * 2), 0).value = dataHBKC0202.PropDtShare.Rows(index).Item("UsrID") & " " & .PropDtShare.Rows(index).Item("UsrNM") & "（" & .PropDtShare.Rows(index).Item("EndUsrBusyoNM") & "）"
    ...
    ''' <summary>
    ''' Excel操作(agree_lend_token)
    ''' </summary>
    ''' <param name="dataHBKC0202">[IN/OUT]インシデント登録（貸出誓約書出力）Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>agree_lend_tokenのExcel出力処理を行う
    ''' <para>作成情報：2012/08/06 s.tsuruta
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Private Function OutputAgreeLendToken(ByVal xlSheets As Object, _
                                       ByVal xlSheet As Object, _
                                       ByRef dataHBKC0202 As DataHBKC0202) As Boolean
        ...
        Try
            ...
            '対象シートをセット
            xlSheet = xlSheets(SHEETNAME_KASHIDASHI_TOKEN)                                                      'シート名（TOKEN）

            With dataHBKC0202

                'シートにデータをセット
                xlSheet.Range(CELLNAME_INCNMB).Value = .PropIntIncNmb                                           '管理番号 
                xlSheet.Range(CELLNAME_KINDCD_KIKINMB).Value = .propStrKindNM & .PropStrKikiNmb                 '機器管理番号
                xlSheet.Range(CELLNAME_MAKER_KISYUNM).Value = .PropStrMaker & .PropStrKisyuNM                   '貸出品名
                xlSheet.PageSetup.RightFooter = xlSheet.PageSetup.RightFooter & PropUserName                    '作業担当(フッタ)
    ...
    ''' <summary>
    ''' Excel操作(agree_lend_other)
    ''' </summary>
    ''' <param name="dataHBKC0202">[IN/OUT]インシデント登録（貸出誓約書出力）Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>agree_lend_otherのExcel出力処理を行う
    ''' <para>作成情報：2012/08/06 s.tsuruta
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Private Function OutputAgreeLendOther(ByVal xlSheets As Object, _
                                       ByVal xlSheet As Object, _
                                       ByRef dataHBKC0202 As DataHBKC0202) As Boolean
        ...
        Try
            ...
            '対象シートをセット
            xlSheet = xlSheets(SHEETNAME_KASHIDASHI_OTHER)                                                      'シート名（OTHER）
