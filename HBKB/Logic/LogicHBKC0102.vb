...
''' <summary>
''' インシデント検索一覧Excel出力ロジッククラス
''' </summary>
''' <remarks>インシデント検索一覧Excel出力Logicクラス
''' <para>作成情報：2012/08/03 y.ikushima
''' <p>改定情報：</p>
''' </para></remarks>
Public Class LogicHBKC0102

    'インスタンス作成
    Private sqlHBKC0102 As New SqlHBKC0102
    Private commonLogic As New CommonLogic
    Private commonLogicHBK As New CommonLogicHBK

    ''' <summary>
    ''' インシデント検索一覧（EXCEL出力）メイン処理
    ''' </summary>
    ''' <param name="dataHBKC0102">[IN/OUT]インシデント検索一覧（EXCEL出力）Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>インシデント検索一覧（EXCEL出力）メイン処理を行う
    ''' <para>作成情報：2012/08/03 y.ikushima
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Public Function CreateOutPutFileMain(ByRef dataHBKC0102 As DataHBKC0102) As Boolean

        '開始ログ出力
        commonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)

        'インシデントデータ取得
        If CreateOutFileForSearch(dataHBKC0102) = False Then
            Return False
        End If

        'ファイル作成処理
        If SetOutPutDataForExcelIncident(dataHBKC0102) = False Then
            Return False
        End If
    ...
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Public Function CreateOutFileForSearch(ByRef dataHBKC0102 As DataHBKC0102) As Boolean
        '開始ログ出力
        commonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)
        ...
        Try
            ...
            If sqlHBKC0102.SetSelectIncidentInfoSql(Adapter, Cn, dataHBKC0102) = False Then
                Return False
            End If

            'ログ出力
            commonLogic.WriteLog(Common.LogLevel.DEBUG_Lv, "インシデント検索一覧データ取得", Nothing, Adapter.SelectCommand)

            'データを取得
            Adapter.Fill(dtOutPut)

            'データクラスに保存
            dataHBKC0102.PropDtResult = dtOutPut
    ...
    ''' <summary>
    ''' インシデント検索一覧Excel出力データ出力処理
    ''' </summary>
    ''' <param name="dataHBKC0102">[IN/OUT]インシデント検索一覧（EXCEL出力）Dataクラス</param>
    ''' <returns>Boolean True:正常終了 False:異常終了</returns>
    ''' <remarks>DBから取得したインシデント検索一覧Excel出力データをExcelにセットする
    ''' <para>作成情報：2012/08/03 y.ikushima
    ''' <p>改訂情報 : </p>
    ''' </para></remarks>
    Public Function SetOutPutDataForExcelIncident(ByRef dataHBKC0102 As DataHBKC0102) As Boolean
        '開始ログ出力
        commonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)
        ...
        Dim strOutPutFilePath As String = dataHBKC0102.PropStrOutPutFilePath '出力先ファイルパス
        ...
        strFormatFilePath = Path.Combine(strFormatFilePath, FORMAT_INCIDENT_SEARCH)
        ...
        Try
            ...
            'データ行数分ループ
            With dataHBKC0102.PropDtResult
            ...
            strOutPutLogFileName = PropUserId & "_" & _
                                    ...
                                    dataHBKC0102.PropStrOutPutFileName
        ...
        Finally
            ...
            dataHBKC0102.PropDtResult.Dispose()
        End Try
    End Function
